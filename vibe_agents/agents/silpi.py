...
async def implement(task_id: str, task_context: str, branch: str, ctx: Context) -> None:
    ...
    await run_agent(
        ...
        (
            ...
            f"3. Run the project's quality gates (check CLAUDE.md for commands)"
            f" — all must pass before committing.\n"
            f"4. Build the project (check CLAUDE.md for the build command)"
            f" — the build MUST succeed with zero errors before proceeding.\n"
            f"5. Commit all changes using the format: {task_id}: <brief description>\n"
            f"6. When done, submit for review:\n"
