...
async def init_project(ctx: Context) -> None:
    ...
    await run_agent(
        ...
        (
            ...
            f"   **Bug labels:**\n"
            f"   > When creating a bd bug task manually, always add the `manual` label:\n"
            f"   > `bd create \"<title>\" --type bug --labels manual`\n"
            f"   > Bugs created automatically by Parikshaka (the e2e QA agent) carry the"
            f" `parikshaka` label instead. This makes it easy to distinguish human-reported"
            f" bugs from automated regression reports:\n"
            f"   > - `manual` — reported by a human\n"
            f"   > - `parikshaka` — detected by the QA agent after a task merge\n\n"
