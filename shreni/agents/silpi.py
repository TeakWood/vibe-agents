...
async def init_project(ctx: Context) -> None:
    ...
    await run_agent(
        ...
        (
            ...
            f"9. **Issue tracker rules** — include this rule verbatim:\n\n"
            f"   > When creating a bd bug task manually, always add the `manual` label:\n"
            f"   > `bd create \"<title>\" --type bug --labels manual`\n"
            f"   > This distinguishes human-reported bugs from automated bugs created by"
            f" Parikshaka (the e2e QA agent). Parikshaka never adds the `manual` label.\n\n"
